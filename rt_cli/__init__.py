...
作为包导入时，自动从根 rt.py 加载核心类（RootHandler 等）
以 python -m rt_cli 运行时，进入 CLI 命令模式
"""
import importlib.util, sys, os

# ── 确保项目根在 sys.path（rt.py 依赖 agent_loop 等）──
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

# ── 从根 rt.py 重新导出核心符号（agentmain.py 等依赖）──
_root_ga = os.path.join(_PROJECT_ROOT, 'rt.py')
if os.path.exists(_root_ga):
    ...
    # 公开导出
    _EXPORT_NAMES = [
        'RootHandler', 'smart_format', 'get_global_memory',
