...
def probe_models(provider, apikey, apibase=None):
    ...
    if ptype == 'native_claude':
        ...
        headers = {'x-api-key': apikey, 'anthropic-version': '2023-06-01', 'User-Agent': 'Root/1.0'}
    else:
        url = f"{base}/models"
        headers = {'Authorization': f'Bearer {apikey}', 'User-Agent': 'Root/1.0'}
...
def generate_mykey(llm_cfgs, platform_configs):
    ...
    lines = []
    lines.append("# ══════════════════════════════════════════════════════════════════════════════")
    lines.append(f"#  Root — mykey.py (由 configure.py 自动生成 @ {datetime.now().strftime('%Y-%m-%d %H:%M')})")
    lines.append("# ══════════════════════════════════════════════════════════════════════════════")
    lines.append("")
    lines.append("# ── 停止符 ──────────────────────────────────────────────────────────────────")
