...
也可以走最朴素的 ZIP：[GitHub 仓库页面](https://github.com/lsdefine/Root) → 点绿色 **Code** → **Download ZIP** → 解压到喜欢的位置。
...
> 💡 这就是 Root 的核心设计理念：**代码即文档**。Agent 能读懂自己的源码，所以任何功能你都可以直接问它。

---

## 💡 使用越久越强

Root 不预设技能，而是**靠使用进化**。每完成一个新任务，它会自动将执行路径固化为 Skill，下次遇到类似任务直接调用。
